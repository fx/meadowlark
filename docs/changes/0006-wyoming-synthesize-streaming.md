...
- [x] `tts.StreamSession`
  - [x] `NewStreamSession`, `Active`, `Start`, `Chunk`, `Compat`, `Stop`, `Close` per the API above (PR #TBD)
  - [x] Session context via `context.WithCancel` from the `Start` call's `ctx`; cancel on `Close`, error, and idle timeout (PR #TBD)
  - [x] Ordered job FIFO with `maxInFlight = 2` prefetch and a single emitter goroutine (PR #TBD)
  - [x] Override-form detection on the session's first non-whitespace character: `{` or `[` suspends segmentation until `synthesize-stop`, then `ParseInput` the whole message and segment its `Input`; anything else skips `ParseInput` entirely and segments incrementally (PR #TBD)
  - [x] Resolve once, before the first segment is synthesized; reuse the plan for all later segments (PR #TBD)
  - [x] Format recording and R8 mismatch abort (PR #TBD)
  - [x] Three-state machine from R5: `idle` / `open` / `terminated`, with `Active()` and `Compat()` both treating `terminated` as absorbing (PR #TBD)
  - [x] Error ownership: session methods emit their own Wyoming `error` and return `nil` for every domain failure; a non-nil return means only that a connection write failed (PR #TBD)
  - [x] Test that an upstream failure yields exactly one `error` event with code `tts-error` and no `handler-error` (PR #TBD)
  - [x] Test that an input `model` override reaches every segment's upstream request while the endpoint stays the one resolved from the start voice (PR #TBD)
  - [x] Terminator discipline from R10, including `audio-stop` before `error` when a segment fails after `audio-start`, and the tombstone that absorbs the compatibility `synthesize` after an early failure (PR #TBD)
  - [x] Test that a failure occurring *before* the compatibility `synthesize` still suppresses it — the message MUST NOT be synthesized a second time (PR #TBD)
  - [x] Idle timer: armed on `Start`, reset by `Chunk` and `Compat`, disarmed by `Stop`, `synthesize-timeout` error on expiry, disabled entirely when the timeout is `0` (PR #TBD)
  - [x] Test that a session receiving one chunk and then stalling is abandoned after the timeout, and that a session receiving a chunk every half-timeout is not (PR #TBD)
  - [x] A single `quiesce` helper implementing R5's three ordered steps — cancel, **join the emitter**, discard the queue — used by the restart, failure, timeout and teardown paths alike, so none of them can skip the join (PR #TBD)
  - [x] The emitter writes the closing `audio-stop` for a group it opened, as its own final act on cancellation; no other goroutine writes audio events, and no terminating path writes that `audio-stop` itself (PR #TBD)
  - [x] `synthesize-start` while active: quiesce, emit `synthesize-stopped`, restart (PR #TBD)
  - [x] Test under `-race` that a restart mid-audio emits the old group's `audio-stop`, then `synthesize-stopped`, and that no old-session audio event appears afterwards or interleaves with the new session's (PR #TBD)
  - [x] Zero-chunk fallback text handling from R7 (PR #TBD)
  - [x] Tests in `internal/tts/stream_session_test.go`: ordered emission with prefetch; suppression and fallback; buffered and streaming endpoint modes; format mismatch; upstream error before and after `audio-start`; cancellation closes bodies; idle timeout; JSON-form and tag-form input arriving in fragments; prose never override-parsed; `-race` clean (PR #TBD)
- [ ] Configuration
