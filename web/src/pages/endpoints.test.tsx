...
describe('EndpointsPage', () => {
  ...
  it('tests connectivity and shows success', async () => {
    const user = userEvent.setup()
    ...
    await waitFor(() => {
      expect(screen.getByText('Test: OK (42ms)')).toBeInTheDocument()
    })
  })

  it('tests connectivity and shows failure', async () => {
    const user = userEvent.setup()
    const fetchMock = vi.fn().mockImplementation((url: string, opts?: RequestInit) => {
      if (url.endsWith('/test') && opts?.method === 'POST') {
        return Promise.resolve({
    ...
    await user.click(screen.getByRole('button', { name: 'Test OpenAI' }))
    await waitFor(() => {
      expect(screen.getByText('Test: Connection refused')).toBeInTheDocument()
    })
  })

  it('tests connectivity and handles network error', async () => {
    const user = userEvent.setup()
    const fetchMock = vi.fn().mockImplementation((url: string, opts?: RequestInit) => {
      if (url.endsWith('/test') && opts?.method === 'POST') {
  ...
  it('discovers voices and shows results', async () => {
    ...
    const fetchMock = vi.fn().mockImplementation((url: string) => {
      if (url.endsWith('/voices')) {
        return Promise.resolve({
          ok: true,
          status: 200,
          json: () => Promise.resolve(['alloy', 'echo', 'fable']),
        })
      }
      return Promise.resolve({ ok: true, status: 200, json: () => Promise.resolve(mockEndpoints) })
    })
    ...
    await waitFor(() => {
      expect(screen.getByText('Voices: alloy, echo, fable')).toBeInTheDocument()
    })
  })
  ...
  it('test result shows Failed when error is missing', async () => {
    const user = userEvent.setup()
    const fetchMock = vi.fn().mockImplementation((url: string, opts?: RequestInit) => {
      if (url.endsWith('/test') && opts?.method === 'POST') {
        return Promise.resolve({
