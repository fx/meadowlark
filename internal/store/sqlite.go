...
func (s *SQLiteStore) ListEndpoints(ctx context.Context) ([]model.Endpoint, error) {
	rows, err := s.db.QueryContext(ctx, `SELECT id, name, base_url, api_key, models, default_voice, default_speed, default_instructions, default_response_format, enabled, streaming_enabled, stream_sample_rate, created_at, updated_at FROM endpoints ORDER BY name`)
	if err != nil {
...
func (s *SQLiteStore) GetEndpoint(ctx context.Context, id string) (*model.Endpoint, error) {
	row := s.db.QueryRowContext(ctx, `SELECT id, name, base_url, api_key, models, default_voice, default_speed, default_instructions, default_response_format, enabled, streaming_enabled, stream_sample_rate, created_at, updated_at FROM endpoints WHERE id = ?`, id)
...
func (s *SQLiteStore) CreateEndpoint(ctx context.Context, e *model.Endpoint) error {
	...
	e.CreatedAt = now
	e.UpdatedAt = now
	_, err := s.db.ExecContext(ctx, `INSERT INTO endpoints (id, name, base_url, api_key, models, default_voice, default_speed, default_instructions, default_response_format, enabled, streaming_enabled, stream_sample_rate, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
		e.ID, e.Name, e.BaseURL, e.APIKey, e.Models, e.DefaultVoice, e.DefaultSpeed, e.DefaultInstructions, e.DefaultResponseFormat, e.Enabled, e.StreamingEnabled, e.StreamSampleRate, e.CreatedAt.Format(time.RFC3339), e.UpdatedAt.Format(time.RFC3339))
	if err != nil {
...
func (s *SQLiteStore) UpdateEndpoint(ctx context.Context, e *model.Endpoint) error {
	...
	res, err := s.db.ExecContext(ctx, `UPDATE endpoints SET name = ?, base_url = ?, api_key = ?, models = ?, default_voice = ?, default_speed = ?, default_instructions = ?, default_response_format = ?, enabled = ?, streaming_enabled = ?, stream_sample_rate = ?, updated_at = ? WHERE id = ?`,
		e.Name, e.BaseURL, e.APIKey, e.Models, e.DefaultVoice, e.DefaultSpeed, e.DefaultInstructions, e.DefaultResponseFormat, e.Enabled, e.StreamingEnabled, e.StreamSampleRate, e.UpdatedAt.Format(time.RFC3339), e.ID)
	if err != nil {
