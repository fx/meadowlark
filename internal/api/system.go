...
func (s *Server) ListVoices(w http.ResponseWriter, r *http.Request) {
	...
	// Canonical voices: each enabled endpoint × model produces entries.
	// The voice name in canonical entries uses the model name as the voice identifier
	// since we don't know the actual voice names without querying the endpoint.
	for _, ep := range endpoints {
		if !ep.Enabled {
			continue
		}
