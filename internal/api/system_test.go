...
func (m *mockStore) ListEndpointVoices(_ context.Context, endpointID string) ([]model.EndpointVoice, error) {
...
func (m *mockStore) UpsertEndpointVoices(_ context.Context, endpointID string, voices []model.EndpointVoice) error {
	if m.endpointVoices == nil {
...
func (m *mockStore) SetEndpointVoiceEnabled(_ context.Context, endpointID, voiceID string, enabled bool) (*model.EndpointVoice, error) {
	if m.endpointVoices == nil {
...
func TestListVoices_ReturnsCanonicalVoicesFromEnabledEndpointVoices(t *testing.T) {
	ms := &mockStore{
		endpoints: []model.Endpoint{
			{ID: "ep1", Name: "OpenAI", Models: model.StringSlice{"tts-1", "tts-1-hd"}, Enabled: true},
		},
		endpointVoices: map[string][]model.EndpointVoice{
			"ep1": {{EndpointID: "ep1", VoiceID: "alloy", Enabled: true}},
		},
	}
	srv := newSystemTestServer(ms)
	router := srv.setupRoutes()
	...
	assert.Equal(t, "alloy (OpenAI, tts-1-hd)", body[1].Name)
	assert.Equal(t, "tts-1-hd", body[1].Model)
}

func TestListVoices_FiltersDisabledVoices(t *testing.T) {
	ms := &mockStore{
		endpoints: []model.Endpoint{
...
func TestListVoices_NoEndpointVoicesEmitsNoCanonical(t *testing.T) {
	// An endpoint with no endpoint_voices rows MUST NOT fall back to live probe.
	ms := &mockStore{
		endpoints: []model.Endpoint{
			{ID: "ep1", Name: "OpenAI", Models: model.StringSlice{"tts-1"}, Enabled: true},
		},
...
func TestListVoices_CombinesCanonicalAndAlias(t *testing.T) {
	...
	// 1 canonical (ep1 alloy x tts-1) + 1 enabled alias = 2.
	require.Len(t, body, 2)
	assert.False(t, body[0].IsAlias)
	assert.True(t, body[1].IsAlias)
	assert.Equal(t, "narrator", body[1].Name)
}
